...
public class UserController extends Controller implements IConst {
    ...
    public static Result updateUser(long id) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            found.invoiceTitle = formObj.invoiceTitle;
    ...
    public static Result getDownLineUsers(Long id) {
        play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
                + " | DATA: " + request().body().asJson());
        ...
        // 一级下线
        List<UserModel> records = UserModel.find.where().eq("refUplineUserId", id).orderBy("becomeDownlineTime desc")
                .findList();
        firstDownlineCount = records.size();
        // 二级下线
        for (int i = 0; i < records.size(); i++) {
            List<UserModel> records2 = UserModel.find.where().eq("refUplineUserId", records.get(i).id)
            ...
            // thirdDownlineCount +=
            // UserModel.find.where().eq("refUplineUserId",
            // records2.get(j).id).findList()
            // .size();
            // }
        }
        ...
        if (records.size() > 0) {
            ...
            // msg.message2 = thirdDownlineCount.toString();
