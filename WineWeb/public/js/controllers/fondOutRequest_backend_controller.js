...
var cellSelectionTemplate = "<input ng-class=\"'colt' + col.index\" ng-input=\"COL_FIELD\" ng-model=\"COL_FIELD\"/>";
...
app.controller('FondOutRequestBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    $scope.goJumpPage = function () {
        if ($scope.jumpPage > $scope.pageInfo.total) {
            $scope.jumpPage = $scope.pageInfo.total
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            {field: 'phone', displayName: '联系电话', enableCellEdit: true, editableCellTemplate: cellSelectionTemplate},
            {field: 'bank', displayName: '发卡银行', enableCellEdit: true, editableCellTemplate: cellSelectionTemplate},
            {field: 'subbranchOfBank', displayName: '银行支行', enableCellEdit: true, editableCellTemplate: cellSelectionTemplate},
            {field: 'name', displayName: '持卡人姓名', enableCellEdit: true, editableCellTemplate: cellSelectionTemplate},
            {field: 'cardNumber', displayName: '卡号', enableCellEdit: true, editableCellTemplate: cellSelectionTemplate},
            {field: 'yongJin.toFixed(2)', displayName: '佣金', enableCellEdit: true, editableCellTemplate: cellSelectionTemplate},
            {field: 'createdAtStr', displayName: '提款申请日期'},
            {
                field: 'status',
                displayName: '状态',
                cellTemplate: '<div><div ng-if="COL_FIELD == 0">待结算&nbsp;&nbsp;<input type="button" ng-if="COL_FIELD == 0" value="结算" ng-model="COL_FIELD" ng-click="payment(row.entity)"/></div><div ng-if="COL_FIELD == 1">已结算</div></div>'
            },
            {field: 'comment', displayName: '结算日期'},
        ]
    };
