 ...
 // 搜索
    ...
    $scope.gridOptions = { data: 'list',
        ...
        columnDefs: [
            ...
            {field: 'status', displayName: '订单状态', width: '80', cellTemplate: '<div><div ng-if="COL_FIELD == 0">待付款</div><input type="button" ng-if="COL_FIELD == 0" value="删除" ng-model="COL_FIELD" ng-click="deleteOrder(row.entity)"/><div ng-if="COL_FIELD == 1">已支付</div><input type="button" ng-if="COL_FIELD == 1" value="发货" ng-model="COL_FIELD" ng-click="updateOrderStatus(row.entity)"/><div ng-if="COL_FIELD == 2">已取消</div><input type="button" ng-if="COL_FIELD == 2" value="删除" ng-model="COL_FIELD" ng-click="deleteOrder(row.entity)"/><div ng-if="COL_FIELD == 3">已删除</div><div ng-if="COL_FIELD == 4">已发货</div><div ng-if="COL_FIELD == 5">已确认</div><input type="button" ng-if="COL_FIELD == 5 && row.entity.refResellerId != 0" " value="执行分销" ng-model="COL_FIELD" ng-click="startMarketing(row.entity)"/><div ng-if="COL_FIELD == 6">已计算佣金</div><input type="button" ng-if="COL_FIELD == 6" value="取消分销" ng-model="COL_FIELD" ng-click="cancelMarketing(row.entity)"/><div ng-if="COL_FIELD == 7">已取消计算佣金</div><div ng-if="COL_FIELD == 8">支付失败</div><div ng-if="COL_FIELD == 9">等待支付结果</div></div>'},
